...
Yea, so a couple orders of magnitude faster with vectorization.  Of note is that a modification this trick is used a lot in training deep learning models that want to sample from a categorical distribution (Wav2vec and Dall-E use this).  I'll go over it in another post, but tl;dr, the network learns the probabilities and max is changed to softmax to allow backpropagation.
...
mydf = pd.DataFrame({"col1":np.random.uniform(size=100), "col2":np.random.uniform(size=100)}, index = pd.date_range(start = "11/29/2022", periods=100))
...
Uh, so what happened here?  Well, pandas does subtraction by *index*, like a join, so we just subtracted the values as the same dates, but the first and last dates were missing from col1 and col2 respectively, so we get NaN at those dates.  Clearly this is not what we want.

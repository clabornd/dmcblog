...
In [src/basic-example.py](https://github.com/clabornd/mlflow-examples/blob/master/src/basic-example.py) I use mlflow to very simply log the input parameters, train an sklearn model on practice data, log the trained model as an artifact, and log performance on the test set.  
...
```bash
export MLFLOW_S3_ENDPOINT_URL=<Public IPv4 DNS>:9000 # where the minio service is running
export AWS_ACCESS_KEY_ID=minioadmin # minio username
export AWS_SECRET_ACCESS_KEY=minioadmin # minio password
...
Then set MLFLOW_TRACKING_URI to localhost:5000 and run the training script as above.  Of course if you run the mlflow server on the ec2 instance, you can set the MLFLOW_TRACKING_URI to the `<Public IPv4 DNS>:5000` and add a new inbound rule to the security group to allow traffic on port 5000.

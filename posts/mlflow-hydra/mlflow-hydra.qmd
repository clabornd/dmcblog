...
UPDATE:  This post discusses running the storage (postgres and minio) in docker containers and the mlflow UI from the command line.  I've since edited the associated [github repo](https://github.com/clabornd/mlflow-examples) to run the mlflow UI in a docker container as well from docker-compose, inspired by [this tds post](https://towardsdatascience.com/deploy-mlflow-with-docker-compose-8059f16b6039).  The below should still work, but take a look at that post and the repo for the full docker setup.
